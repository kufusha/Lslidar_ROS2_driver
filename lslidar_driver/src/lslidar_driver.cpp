...
: rclcpp_lifecycle::LifecycleNode("lslidar_node", options)
{
  ...
  current_point_time = 0.0, last_point_time = 0.0, horizontal_angle_resolution = 0.0,
  lidar_number_ = 1, point_num = 2000,
  ...
  std::string ros_package_version =
    "ros2 pkg xml  lslidar_driver | grep '<version>' | grep -oP '(?<=>).*?(?=<)'";
...
bool LslidarDriver::loadParameters()
{
  ...
  if (add_multicast) {
    RCLCPP_INFO(
      this->get_logger(), "opening UDP socket: group_address %s", group_ip_string.c_str());
  }
  return true;
}
...
bool LslidarDriver::createRosIO()
{
  ...
  lslidar_control_service_ = this->create_service<lslidar_msgs::srv::LslidarControl>(
    ...
    rclcpp::ServicesQoS());
  motor_control_service_ = this->create_service<lslidar_msgs::srv::MotorControl>(
    "motor_control",
    std::bind(&LslidarDriver::motorControl, this, std::placeholders::_1, std::placeholders::_2),
    rclcpp::ServicesQoS());
  motor_speed_service_ = this->create_service<lslidar_msgs::srv::MotorSpeed>(
    "set_motor_speed",
    std::bind(&LslidarDriver::motorSpeed, this, std::placeholders::_1, std::placeholders::_2),
    rclcpp::ServicesQoS());
  ...
  data_ip_service_ = this->create_service<lslidar_msgs::srv::DataIp>(
    ...
    rclcpp::ServicesQoS());
  destination_ip_service_ = this->create_service<lslidar_msgs::srv::DestinationIp>(
    "set_destination_ip",
  ...
  time_service_ = this->create_service<lslidar_msgs::srv::TimeService>(
    "time_service",
    std::bind(&LslidarDriver::timeService, this, std::placeholders::_1, std::placeholders::_2),
    rclcpp::ServicesQoS());

  if (dump_file != "") {
    msop_input_.reset(
      new lslidar_driver::InputPCAP(this, msop_udp_port, 1212, packet_rate, dump_file));
  ...
  difop_thread_ =
    std::shared_ptr<std::thread>(new std::thread(std::bind(&LslidarDriver::difopPoll, this)));

  poll_thread_ =
    std::shared_ptr<std::thread>(new std::thread(std::bind(&LslidarDriver::pollThread, this)));
...
void LslidarDriver::difopPoll()
{
  ...
  while (rclcpp::ok()) {
    ...
    if (rc == 0) {
      if (
        difop_packet_ptr->data[0] != 0xa5 || difop_packet_ptr->data[1] != 0xff ||
      ...
      if (is_print_working_time) {
        int total_working_time = (difop_packet_ptr->data[105] << 24) +
          (difop_packet_ptr->data[106] << 16) + (difop_packet_ptr->data[107] << 8) +
        ...
        RCLCPP_INFO(
          this->get_logger(), "total working time: %d hours:%d minutes", total_working_time / 60,
          total_working_time % 60);
        RCLCPP_INFO(
          this->get_logger(), "less than     -40 degrees  working time: %d hours:%d minutes",
          less_minus40_degree_working_time / 60, less_minus40_degree_working_time % 60);
        RCLCPP_INFO(
          this->get_logger(), "-40 degrees ~ -10 degrees  working time: %d hours:%d minutes",
          minus40_to_minus10_working_time / 60, minus40_to_minus10_working_time % 60);
        RCLCPP_INFO(
          this->get_logger(), "-10 degrees ~ 30  degrees  working time: %d hours:%d minutes",
          minus10_to_30_working_time / 60, minus10_to_30_working_time % 60);
        RCLCPP_INFO(
          this->get_logger(), "30  degrees ~ 70  degrees  working time: %d hours:%d minutes",
          positive30_to_70_working_time / 60, positive30_to_70_working_time % 60);
        RCLCPP_INFO(
          this->get_logger(), "70  degrees ~ 100 degrees  working time: %d hours:%d minutes",
...
void LslidarDriver::pointcloudToLaserscan(
  const sensor_msgs::msg::PointCloud2 & cloud_msg, sensor_msgs::msg::LaserScan & output_scan)
{
  ...
  uint32_t ranges_size =
    std::ceil((output_scan.angle_max - output_scan.angle_min) / output_scan.angle_increment);
  ...
  if (pcl_type) {
    ...
    if (!sweep_data_bak->points.empty()) {
      for (j = 0; j < sweep_data_bak->points.size(); ++j) {
        if (
          (sweep_data_bak->points[j].azimuth > angle_disable_min) &&
    ...
    if (!sweep_data_bak->points.empty()) {
      for (j = 0; j < sweep_data_bak->points.size(); ++j) {
        ...
        if (current_point_time - last_point_time < 0.0) {
          // ROS_WARN("timestamp is rolled back! current point time: %.12f  last point time: %.12f",
          // current_point_time, last_point_time);
        }
  ...
  for (size_t block_idx = 0; block_idx < BLOCKS_PER_PACKET; ++block_idx) {
    ...
    for (size_t scan_fir_idx = 0; scan_fir_idx < SCANS_PER_FIRING_CX; ++scan_fir_idx) {
      ...
      firings.azimuth[block_idx * 32 + scan_fir_idx] =
        firings.firing_azimuth[block_idx] + scan_fir_idx * azimuth_diff_b / FIRING_TOFFSET_C8;
      firings.azimuth[block_idx * 32 + scan_fir_idx] =
...
bool LslidarDriver::poll()
{
  ...
  if (use_gps_ts) {
    if (time_service_mode_ == 1) {  // ptp授时
      ...
      uint64_t timestamp_s =
        (pkt.data[1201] * pow(2, 32) + pkt.data[1202] * pow(2, 24) + pkt.data[1203] * pow(2, 16) +
         pkt.data[1204] * pow(2, 8) + pkt.data[1205] * pow(2, 0));
      uint64_t timestamp_nsce = pkt.data[1206] + pkt.data[1207] * pow(2, 8) +
        pkt.data[1208] * pow(2, 16) + pkt.data[1209] * pow(2, 24);  // ns
      ...
      packet_time_ns = pkt.data[1206] + pkt.data[1207] * pow(2, 8) + pkt.data[1208] * pow(2, 16) +
  ...
  for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
    if (!is_new_c32w_ && "c32_70" == c32_type) {
      if (
        fir_idx % 32 == 29 || fir_idx % 32 == 6 || fir_idx % 32 == 14 || fir_idx % 32 == 22 ||
    ...
    if (coordinate_opt) {
      x_coord =
        firings.distance[fir_idx] * cos_scan_altitude[fir_idx % lidar_number_] * cos_azimuth +
        R1 * cos((conversionAngle - firings.azimuth[fir_idx] * 0.01) * DEG_TO_RAD);
      y_coord =
        -firings.distance[fir_idx] * cos_scan_altitude[fir_idx % lidar_number_] * sin_azimuth +
    ...
    lslidar_msgs::msg::LslidarPoint & new_point = sweep_data->points[sweep_data->points.size() - 1];
  ...
  if (end_fir_idx != SCANS_PER_PACKET) {
    ...
    if (last_packet_time > 1e-6) {
      sweep_end_time = current_packet_time -
        (current_packet_time - last_packet_time) * (SCANS_PER_PACKET - end_fir_idx) /
          SCANS_PER_PACKET;
    } else {
      sweep_end_time = current_packet_time;
    }
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
      ...
      sweep_data->points.push_back(lslidar_msgs::msg::LslidarPoint());
      lslidar_msgs::msg::LslidarPoint & new_point =
...
bool LslidarDriver::sendPacketTolidar(unsigned char * config_data) const
{
  ...
  sendto(
    socketid, (const char *)config_data, 1206, 0, (struct sockaddr *)&addrSrv, sizeof(addrSrv));
  return true;
}
  ...
  std::shared_ptr<lslidar_msgs::srv::LslidarControl::Response> res)
{
  ...
  if (
    config_data[0] != 0xa5 || config_data[1] != 0xff || config_data[2] != 0x00 ||
    config_data[3] != 0x5a) {
    res->result = 0;
    RCLCPP_ERROR(this->get_logger(), "Can not get dev packet! Set failed!");
    return true;
  ...
  if (req->laser_control == 1) {
    if ((rc_msop = msop_input_->getPacket(packet0)) == 0) {
      res->result = 1;
      RCLCPP_WARN(
        this->get_logger(), "receive cmd: %d,already power on status", req->laser_control);
      return true;
    }
  ...
  std::shared_ptr<lslidar_msgs::srv::TimeService::Response> res)
{
  ...
  transform(
    time_service_mode.begin(), time_service_mode.end(), time_service_mode.begin(), ::tolower);
  ...
  if (time_service_mode == "gps") {
    ...
    sscanf(
      ntp_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value, &second_value, &third_value, &end_value);
  ...
  std::shared_ptr<lslidar_msgs::srv::DataIp::Response> res)
{
  ...
  unsigned short first_value, second_value, third_value, end_value;
  sscanf(
    req->data_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value, &second_value, &third_value, &end_value);

  std::string destination_ip = std::to_string(config_data[14]) + "." +
    std::to_string(config_data[15]) + "." + std::to_string(config_data[16]) + "." +
    std::to_string(config_data[17]);
  if (
    first_value == 0 || first_value == 127 || (first_value >= 240 && first_value <= 255) ||
  ...
  std::shared_ptr<lslidar_msgs::srv::DestinationIp::Response> res)
{
  ...
  unsigned short first_value, second_value, third_value, end_value;
  sscanf(
    req->destination_ip.c_str(), "%hu.%hu.%hu.%hu", &first_value, &second_value, &third_value,
    &end_value);
