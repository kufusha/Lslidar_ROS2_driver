  ...
  packet_size_(packet_size)
{
  ...
  if (!devip_str_.empty()) {
    RCLCPP_INFO(
      private_nh->get_logger(), "[driver][input] accepting packets from IP address: %s  port: %d",
  ...
  while (flag == 1) {
    ...
    do {
      ...
      if (retval < 0) {  // poll() error?
        if (errno != EINTR) {
          RCLCPP_ERROR(
            private_nh_->get_logger(), "[driver][socket] poll() error: %s", strerror(errno));
        }
        return 1;
      }
      if (retval == 0) {  // poll() timeout?
        ...
        char bufTime[64] = {0};
        snprintf(
          bufTime, sizeof(bufTime), "%d-%d-%d %d:%d:%d", curTm->tm_year + 1900, curTm->tm_mon + 1,
          curTm->tm_mday, curTm->tm_hour, curTm->tm_min, curTm->tm_sec);

        RCLCPP_WARN(
          private_nh_->get_logger(), "%s  lslidar poll() timeout, port: %d", bufTime, port_);
        /*
            ...
            sender_address.sin_port = htons(MSOP_DATA_PORT_NUMBER);  // port in network byte order,
           set any value sender_address.sin_addr.s_addr = devip_.s_addr;          // automatically
           fill in my IP sendto(sockfd_, &buffer_data, strlen(buffer_data), 0,
    ...
    ssize_t nbytes = recvfrom(
      sockfd_, &pkt->data[0], packet_size_, 0, (sockaddr *)&sender_address, &sender_address_len);
    ...
    if (nbytes < 0) {
      ...
      if (!devip_str_.empty() && sender_address.sin_addr.s_addr != devip_.s_addr) {
        RCLCPP_ERROR(
          private_nh_->get_logger(), "lidar IP parameter set error,please reset in launch file");
        continue;
      } else {
        break;  // done
      }
    ...
    RCLCPP_WARN(
      private_nh_->get_logger(), "[driver][socket] incomplete lslidar packet read: %zd bytes",
      nbytes);
  }
  if (flag == 0) {
    abort();
  ...
  filename_(filename)
{
  ...
  if (repeat_delay_ > 0.0) {
    RCLCPP_WARN(
      private_nh_->get_logger(), "Delay %.3f seconds before repeating input file.", repeat_delay_);
  }
...
int InputPCAP::getPacket(lslidar_msgs::msg::LslidarPacket::UniquePtr & pkt)
{
  ...
  while (flag == 1) {
    ...
    if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
      ...
      // selected IP address.
      if (
        !devip_str_.empty() && (0 == pcap_offline_filter(&pcap_packet_filter_, header, pkt_data))) {
        continue;
      }
      ...
      if (
        pkt->data[0] == 0xA5 && pkt->data[1] == 0xFF && pkt->data[2] == 0x00 &&
    ...
    if (empty_) {  // no data in file?
      RCLCPP_WARN(
        private_nh_->get_logger(), "Error %d reading lslidar packet: %s", res, pcap_geterr(pcap_));
      return -1;
    }
    ...
    if (repeat_delay_ > 0.0) {
      RCLCPP_WARN(
        private_nh_->get_logger(), "end of file reached -- delaying %.3f seconds.", repeat_delay_);
